...
def results(q_0, nu, k, method):
    ...
    for i in range(0, (len(a) - 1) // n ):
        ...
        f1d = float(a[i * n + 1])
        #f2d = float(a[i * n + 3])
        x_1.append(np.sin(float(a[i * n])) * l)
        y_1.append(-np.cos(float(a[i * n])) * l + da * np.cos(nu * t[i]))
...
x_1, y_1= results(np.pi + 0.001, 61, 0.2, "rungecutta")#, T1, T2, P, t = results(1, 1, "rungecutta")
...
line_1 = ax.plot(x_1[0], y_1[0], color ='tab:red', label=f'{config["method_type"]}, dt = {config["dt"]}, w = {config["w"]}')[0]
...
ax.set(xlim=[-(l + l), (l + l)], ylim=[-(l + l), (l + l)], xlabel= 'x', ylabel='y')

def update(frame):
    # for each frame, update the data stored on each artist.

    cur_frame = int(frame * frames_factor)
    ...
    line_1.set_xdata(x_1[:cur_frame])
    line_1.set_ydata(y_1[:cur_frame])
...
animation_time = 10#config["time"]

delay = 15

speed_up_factor = 100

frames_factor = delay / config["dt"] / 100000 * speed_up_factor
